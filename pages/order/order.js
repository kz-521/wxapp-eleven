...
Page({
    ...
    onLoad: async function (options) {
        ...
        for (let i = 0; i<orderItems.length; i++){
            ...
            packMoney += orderItems[i].count* orderItems[i].packMoney

        }

        console.log(packMoney)
    ...
    remarkValue(event){
        let value = event.currentTarget.dataset.keyword

        let that = this
        const remark = that.data.remark+value

        that.setData({
            remark
        })
    },
    customBackReturnListener(){
        wx.navigateBack({
            delta: 1
        })
    }
